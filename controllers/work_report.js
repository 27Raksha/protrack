...
const endDate = today.endOf('isoWeek').format('YYYY-MM-DD'); // Get the start of the current ISO week (Monday)
...
console.log(`Description Array ========================================= >>>>>>>> ${descriptionArray}`);  
       ...
       return res.status(200).send({message: "OK", task_description: descriptionArray});
          ...
          return res.status(200).send({message: "OK",obj: JSON.stringify(results)});

        // const activity = await sequelize2.query(
        //     "SELECT * FROM activity WHERE activity_name = :activity_name LIMIT 1",
        //     {
        //         replacements: { activity_name: reqActivity },
        //         type: sequelize2.QueryTypes.SELECT,
        //     }
        // );
        // const activityObject = activity[0];
